...
on('IssueEvent', function (e)
    if (e.action == 'opened') then
        ...
        translate(e.title, 'en', function(translatedTitle)
            local commentHeader = ''
            local commentTitle = ''
--[[            local commentHeader = rendStr(config['issue-english-translator'].header, {author=e.author})
            local commentTitle = rendStr(config['issue-english-translator'].title, {title=translatedTitle})]]


            local commentHeader = rendStr(compConfig.header, {author=e.author})
            local commentTitle = rendStr(compConfig.title, {title=translatedTitle})
               ...
               translate(e.body, 'en', function(translatedBody)
                ...
                then
                    local commentBody = ''
                    local commentBody = rendStr(compConfig.body, {body=translatedBody})
